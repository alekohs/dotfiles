...
local group = augroup("Lsp", { clear = true })
...
if vim.g.inlay_focus_off == nil then
  vim.g.inlay_focus_off = false
end

if not vim.g.inlay_focus_toggle_mapped then
  vim.keymap.set("n", "<leader>uh", function()
    vim.g.inlay_focus_off = not vim.g.inlay_focus_off
    set_inlay_hints(not vim.g.inlay_focus_off)
    vim.notify("Inlay hints " .. (vim.g.inlay_focus_off and "OFF (focus mode)" or "ON"))
  end, { desc = "Toggle inlay hints (focus mode)" })

  vim.g.inlay_focus_toggle_mapped = true
end
...
aucmd("LspAttach", {
  ...
  callback = function(args)
    ...
    if is_roslyn_client(client) then
      ...
      if not vim.b[bufnr].roslyn_auto_insert_attached then
        aucmd("InsertCharPre", {
        ...
        vim.b[bufnr].roslyn_auto_insert_attached = true
      end
